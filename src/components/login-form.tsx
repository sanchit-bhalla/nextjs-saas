...
export function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await signIn(CREDENTIAL_PROVIDER_ID, {
        email,
        password,
        redirect: false, // Prevent automatic redirect
      });

      // console.log("Inside login-form : ", result);
      if (result?.error) {
  ...
  useEffect(() => {
    ...
    if (oauthError === "OAuthAccountNotLinked") {
      ...
      toast.error("Google login was cancelled or permissions were denied.");
    } else if (oauthError === "MissingEmail") {
      toast.error("Google login failed: email not found in profile.");
